...
public class SsimCalculator {
	...
	public SsimCalculator(final File referenceFile) throws SsimException,
			IOException {
	...
	private WindowManager getWindowManager(final byte[] compBytes,
			final int winSize) throws SsimException, IOException {
		final BufferedImage compImg = getBufferedImageForBytes(compBytes);

		if (getReferenceImage().getColorModel().getPixelSize() != compImg
				.getColorModel().getPixelSize()) {
			throw new SsimException("bits per pixel of images don't match");
		}

		return new WindowManager(getReferenceImage(), compImg, winSize);
	}

	public double compareTo(final File comp, final int winSize)
			throws SsimException, IOException {
		final byte[] compBytes = loadFile(comp);
		final WindowManager manager = getWindowManager(compBytes, winSize);

		final int[] size = getReferenceImage().getColorModel()
				.getComponentSize();
		...
		final Iterator<Pair<Window>> iterator = manager.getWindowContainer()
				.iterator();
		...
		while (iterator.hasNext()) {
			...
			final double denominator = (pow(mx, 2) + pow(my, 2) + c1)
					* (sigsqx + sigsqy + c2);
	...
	private BufferedImage getBufferedImageForBytes(final byte[] imageInBytes)
			throws IOException {
	...
	private byte[] loadFile(final File fileToLoad) throws IOException {
