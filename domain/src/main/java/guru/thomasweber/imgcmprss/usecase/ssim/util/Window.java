...
class Window {
	...
	private double[] calculateLumaValuesForWindow(BufferedImage image) {
		final double[] winLumaValues = new double[getWindowSize() * getWindowSize()];
		...
		for (int i = getMinX(); i < getMaxX(); i++) {
			for (int j = getMinY(); j < getMaxY(); j++) {
				...
				final double Y = ((double) red * RED_COEFFICIENT) + ((double) green * GREEN_COEFFICIENT)
						+ ((double) blue * BLUE_COEFFICIENT);

				winLumaValues[counter] = Y;
				counter++;
			}
		}

		return winLumaValues;
	}
